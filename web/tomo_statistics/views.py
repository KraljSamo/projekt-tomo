...
from attempts.models import HistoricalAttempt, Attempt
...
def compare_solutions(request, course_pk):
    ...
    problem_set = get_object_or_404(ProblemSet, pk=problem_set_pk) if problem_set_pk is not None else None
    first_student = get_object_or_404(User, pk=first_student_pk) if first_student_pk is not None else None
    second_student = get_object_or_404(User, pk=second_student_pk) if second_student_pk is not None else None
    ...
    if compare_type == 'timeline':
        ...
        attempts = HistoricalAttempt.objects.filter(
            ...
            ).order_by('history_date').prefetch_related('part', 'part__problem')
        ...
        if problem_set is not None:
            ...
            problems = {problem : problem.parts.all() for problem in problem_set.problems.all()}
            for problem in problems:
                for part in problems[problem]:
                    try:
                        part.attempt_student1 = attempts.get(user=first_student, part=part)
                    except:
                        part.attempt_student1 = None
                    try:
                        part.attempt_student2 = attempts.get(user=second_student, part=part)
                    except:
